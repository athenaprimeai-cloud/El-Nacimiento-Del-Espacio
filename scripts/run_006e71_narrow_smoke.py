...
from run_006e67_narrow_smoke import MATRIX as MATRIX_006E67


PHASE_ID = "006E71"
RESULT_PASS = "006E71_NEXT_NARROW_FIXED_SEMANTIC_SMOKE_PASS_LIMITED"
RESULT_WARNINGS = "006E71_NEXT_NARROW_FIXED_SEMANTIC_SMOKE_PASS_WITH_WARNINGS"
RESULT_BLOCKED = "006E71_BLOCKED_ENVIRONMENT_OR_CAPTURE"
RESULT_INCONCLUSIVE = "006E71_INCONCLUSIVE_SEMANTICS_OR_CAPTURE"
RESULT_FAIL_SCOPE = "006E71_FAIL_SCOPE_OR_SEMANTICS"
...
ARTIFACT_DIR = Path("artifacts/006E71-next-narrow-fixed-semantic-ledger")
PRECISIONS = [96, 128]
EXPECTED_RECORDS = 90
...
def replay_row_from_006e67(row: dict) -> dict:
    replay = dict(row)
    replay["source"] = "replay_006E67"
    return replay


MATRIX = [
    *[replay_row_from_006e67(row) for row in MATRIX_006E67],
    {"label": "LBOX_P1_C_YOCTO", "source": "new_006E71", "block": "CENTER_YOCTO_TIGHT", "parent": "LBOX_P1_C_ZEPTO", "real_mid": "1/2", "real_radius": "1/2048000", "imag_mid": "7/5", "imag_radius": "1/4096000"},
    {"label": "LBOX_P2_C_YOCTO", "source": "new_006E71", "block": "CENTER_YOCTO_TIGHT", "parent": "LBOX_P2_C_ZEPTO", "real_mid": "3/4", "real_radius": "1/4096000", "imag_mid": "2/1", "imag_radius": "1/2048000"},
    {"label": "LBOX_P3_C_YOCTO", "source": "new_006E71", "block": "CENTER_YOCTO_TIGHT", "parent": "LBOX_P3_C_ZEPTO", "real_mid": "1/3", "real_radius": "1/3072000", "imag_mid": "5/3", "imag_radius": "1/3072000"},
]
...
def build_diagnostic_pairs() -> list[tuple[str, str, str]]:
    ...
    for prefix in ("LBOX_P1", "LBOX_P2", "LBOX_P3"):
        ...
        diagnostic_pairs.extend([
            ...
            ("zepto_tight_to_yocto_tight_children", prefix + "_C_ZEPTO", prefix + "_C_YOCTO"),
        ])
...
def validate_static_contract() -> list[str]:
    ...
    if MATRIX_ID != "006E71_REPLAY_PLUS_CENTER_YOCTO_TIGHT":
        errors.append("matrix id drifted")
    if ARTIFACT_DIR != Path("artifacts/006E71-next-narrow-fixed-semantic-ledger"):
    ...
    replay_rows = [row for row in MATRIX if row.get("source") == "replay_006E67"]
    yocto_rows = [row for row in MATRIX if row.get("source") == "new_006E71"]
    if len(replay_rows) != 42:
        errors.append("replay row count is not 42")
    if len(yocto_rows) != 3:
        errors.append("yocto row count is not 3")
    ...
    actual_yocto = {
        (row["label"], row["parent"], row["real_mid"], row["real_radius"], row["imag_mid"], row["imag_radius"])
        for row in yocto_rows
    }
    if actual_yocto != expected_yocto:
        errors.append("yocto descriptors do not match 006E70")
...
def main() -> int:
    ...
    new_yocto_labels = {row["label"] for row in MATRIX if row["source"] == "new_006E71"}
    ...
    manifest = {
        ...
        "new_center_yocto_tight_inputs": len(new_yocto_labels),
        ...
        "new_006E71_yocto_labels": len(new_yocto_labels),
