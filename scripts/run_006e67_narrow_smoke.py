...
from run_006e63_narrow_smoke import MATRIX as MATRIX_006E63


PHASE_ID = "006E67"
RESULT_PASS = "006E67_NEXT_NARROW_FIXED_SEMANTIC_SMOKE_PASS_LIMITED"
RESULT_WARNINGS = "006E67_NEXT_NARROW_FIXED_SEMANTIC_SMOKE_PASS_WITH_WARNINGS"
RESULT_BLOCKED = "006E67_BLOCKED_ENVIRONMENT_OR_CAPTURE"
RESULT_INCONCLUSIVE = "006E67_INCONCLUSIVE_SEMANTICS_OR_CAPTURE"
RESULT_FAIL_SCOPE = "006E67_FAIL_SCOPE_OR_SEMANTICS"
...
ARTIFACT_DIR = Path("artifacts/006E67-next-narrow-fixed-semantic-ledger")
PRECISIONS = [96, 128]
EXPECTED_RECORDS = 84
...
def replay_row_from_006e63(row: dict) -> dict:
    replay = dict(row)
    replay["source"] = "replay_006E63"
    return replay


MATRIX = [
    *[replay_row_from_006e63(row) for row in MATRIX_006E63],
    {"label": "LBOX_P1_C_ZEPTO", "source": "new_006E67", "block": "CENTER_ZEPTO_TIGHT", "parent": "LBOX_P1_C_ATTO", "real_mid": "1/2", "real_radius": "1/1024000", "imag_mid": "7/5", "imag_radius": "1/2048000"},
    {"label": "LBOX_P2_C_ZEPTO", "source": "new_006E67", "block": "CENTER_ZEPTO_TIGHT", "parent": "LBOX_P2_C_ATTO", "real_mid": "3/4", "real_radius": "1/2048000", "imag_mid": "2/1", "imag_radius": "1/1024000"},
    {"label": "LBOX_P3_C_ZEPTO", "source": "new_006E67", "block": "CENTER_ZEPTO_TIGHT", "parent": "LBOX_P3_C_ATTO", "real_mid": "1/3", "real_radius": "1/1536000", "imag_mid": "5/3", "imag_radius": "1/1536000"},
]
...
def build_diagnostic_pairs() -> list[tuple[str, str, str]]:
    diagnostic_pairs: list[tuple[str, str, str]] = []
    for prefix in ("LBOX_P1", "LBOX_P2", "LBOX_P3"):
        ...
        diagnostic_pairs.extend([
            ...
            ("atto_tight_to_zepto_tight_children", prefix + "_C_ATTO", prefix + "_C_ZEPTO"),
        ])
...
def validate_static_contract() -> list[str]:
    ...
    if MATRIX_ID != "006E67_REPLAY_PLUS_CENTER_ZEPTO_TIGHT":
        errors.append("matrix id drifted")
    if ARTIFACT_DIR != Path("artifacts/006E67-next-narrow-fixed-semantic-ledger"):
        errors.append("artifact directory drifted")
    if PRECISIONS != [96, 128]:
        errors.append("precision values drifted")
    if EXPECTED_RECORDS != 84:
        errors.append("expected record count drifted")
    if EXPECTED_DIAGNOSTICS != 78:
        errors.append("expected diagnostic count drifted")
    if len(MATRIX) != 42:
        errors.append("matrix size is not 42")

    replay_rows = [row for row in MATRIX if row.get("source") == "replay_006E63"]
    zepto_rows = [row for row in MATRIX if row.get("source") == "new_006E67"]
    if len(replay_rows) != 39:
        errors.append("replay row count is not 39")
    if len(zepto_rows) != 3:
        errors.append("zepto row count is not 3")
    ...
    actual_zepto = {
        (row["label"], row["parent"], row["real_radius"], row["imag_radius"])
        for row in zepto_rows
    }
    if actual_zepto != expected_zepto:
        errors.append("zepto descriptors do not match 006E66")
...
def main() -> int:
    ...
    new_zepto_labels = {row["label"] for row in MATRIX if row["source"] == "new_006E67"}
    ...
    manifest = {
        ...
        "new_center_zepto_tight_inputs": len(new_zepto_labels),
        ...
        "new_006E67_zepto_labels": len(new_zepto_labels),
